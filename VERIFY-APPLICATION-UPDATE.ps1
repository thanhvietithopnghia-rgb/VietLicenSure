...
try {
    ...
    Assert-UpdateTest ($candidate.LatestVersion -eq '5.0.0.1' -and $candidate.Title -eq 'Test release VI') 'Manifest result or localization is invalid.'
    ...
    $replacementBuild = ConvertFrom-ToolUpdateManifest -Manifest $currentManifest -InstalledVersion '5.0.0.1' -InstalledSha256 ('B' * 64) -SelectedCulture 'vi-VN'
    ...
    Assert-UpdateThrows { ConvertFrom-ToolUpdateManifest -Manifest $currentManifest -InstalledVersion '5.0.0.1' -InstalledSha256 'INVALID' | Out-Null } 'An invalid installed executable SHA-256 was accepted.'
    ...
    $deniedArguments = "-NoProfile -ExecutionPolicy RemoteSigned -File `"$updateManagerPath`" -Mode Check -CurrentVersion 5.0.0.1 -ResultFile `"$deniedResultPath`""
    ...
    $developmentDeniedArguments = "-NoProfile -ExecutionPolicy RemoteSigned -File `"$updateManagerPath`" -Mode Check -ConsentGranted -CurrentVersion 5.0.0.1 -ResultFile `"$developmentDeniedResultPath`""
    ...
    $installResult = Install-ToolUpdateExecutable -StagedPath $stagedPath -TargetPath $targetPath -TargetSha256 $newSha256 -InstalledVersion '4.8.0.1' -TargetVersion '5.0.0.1' -CacheDirectory $cacheDirectory -SkipRestart
