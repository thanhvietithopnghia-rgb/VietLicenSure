...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("VietLicenSure-v5.0-extensions-" + [Guid]::NewGuid().ToString("N"))
...
$validation = Test-ToolReportEnvelope -Report $json -ExpectedReportKind "CertificateAudit" -ExpectedToolVersion "4.8"
    ...
    try {
        ...
        if ($tempFull.StartsWith($expectedPrefix, [StringComparison]::OrdinalIgnoreCase) -and
            [IO.Path]::GetFileName($tempFull).StartsWith("VietLicenSure-v5.0-extensions-", [StringComparison]::OrdinalIgnoreCase) -and
