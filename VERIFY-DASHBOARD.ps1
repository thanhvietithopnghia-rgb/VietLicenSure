...
Assert-SourcePattern $text 'BorderColor\s*=\s*if\s*\([$]dark\)' 'Palette trạng thái chưa có viền riêng theo giao diện sáng/tối.'
Assert-SourcePattern $text '[$]card\.Add_Paint\(' 'Các thẻ trạng thái chưa tự vẽ khung màu.'
Assert-SourcePattern $text '[$]cardRecord\.Panel\.Tag\.BorderColor\s*=\s*[$]statusPalette\.BorderColor' 'Theme dashboard chưa đồng bộ màu khung theo từng thẻ trạng thái.'
...
if (-not (Test-Path -LiteralPath $guideViPath -PathType Leaf) -or
    ...
    if ($historyText -notmatch '(?m)^##\s+v5\.0\s+—\s+08/09/2026\s*$' -or
    ...
    $v5HistorySection = [regex]::Match($historyText, '(?s)(?m)^##\s+v5\.0\s+—\s+08/09/2026\s*$.*?(?=^##\s+v4\.9)').Value
