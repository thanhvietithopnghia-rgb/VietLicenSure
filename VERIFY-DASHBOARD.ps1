...
Assert-SourcePattern $text '[$]releaseVersion\s*=\s*"5\.0"' 'Dashboard chưa dùng release 5.0.'
Assert-SourcePattern $text '[$]releaseBuildDate\s*=\s*"2026\.09\.25"' 'Dashboard chưa dùng ngày build 2026.09.25.'
...
Assert-SourcePattern $text 'AccentColor\s*=\s*if\s*\([$]dark\)' 'Palette trạng thái chưa có màu viền riêng theo giao diện sáng/tối.'
Assert-SourcePattern $text '[$]card\.Add_Paint\(' 'Các thẻ trạng thái chưa tự vẽ khung màu.'
...
foreach ($aboutCase in @(@('vi-VN', $fitViCatalog), @('en-US', $fitEnCatalog))) {
    foreach ($dpiScale in @(1.0, 1.25, 1.5, 1.75, 2.0)) {
        ...
        try {
            ...
            $taglineText = [string]::Format([Globalization.CultureInfo]::InvariantCulture, [string]$aboutCase[1].'about.byline', @('v5.0','2026.09.25'))
...
if (-not (Test-Path -LiteralPath $guideViPath -PathType Leaf) -or
    ...
    if ($historyText -notmatch '(?m)^##\s+v5\.0\s+—\s+cập nhật\s+25/09/2026\s+\(phát hành lần đầu 08/09/2026\)\s*$' -or
    ...
    $v5HistorySection = [regex]::Match($historyText, '(?s)(?m)^##\s+v5\.0\s+—\s+cập nhật\s+25/09/2026\s+\(phát hành lần đầu 08/09/2026\)\s*$.*?(?=^##\s+v4\.9)').Value
    if ([regex]::Matches($v5HistorySection, '(?m)^-\s+').Count -ne 10 -or
    ...
    $requiredHistoryHeadings = @(
        '## v5.0 — cập nhật 25/09/2026 (phát hành lần đầu 08/09/2026)',
        '## v4.9.0.0 — 22/08/2026',
        '## v4.8.0.1 — 18/08/2026',
        '## v4.8.0.0 — 10/08/2026',
        '## v4.6 — 06/08/2026',
    ...
    foreach ($requiredEnglishHistoryHeading in @(
        '## v5.0 — updated September 25, 2026 (first released September 8, 2026)',
...
foreach ($navigationPattern in @(
    ...
    '(?s)function\s+Show-ProcessingTimelineWindow.+?Show\([$]form\)',
    '(?s)function\s+Write-ProgressLog.+?currentTaskProgressEntries\.Add.+?Add-ProcessingTimelineEntry',
    '(?s)function\s+Start-CleanupDeep.+?Show-ProcessingTimelineWindow\s+-Action\s+[$]actionText',
    ...
    '(?s)function\s+Show-CleanupResultCenter.+?hasExplicitSelection.+?postVerificationSuggestedIds\.Count\s+-eq\s+0.+?Code\s+-ne\s+''RemediateRemaining''',
    '(?s)postVerificationSuggestedIds\.Count\s+-eq\s+0.+?cleanup\.remediation\.noRemainingSelectedLog.+?continue.+?-RestoreExactSelection',
