...
if ([string]$metadata.ContractSchemaVersion -ne "1.0" -or [string]$metadata.ResultSchemaVersion -ne "1.0" -or [string]$metadata.ToolVersion -ne "5.0") { throw "Metadata hợp đồng mô-đun không hợp lệ." }
...
if ([string]$descriptor.ContractSchemaVersion -ne "1.0" -or [string]$descriptor.ResultSchemaVersion -ne "1.0" -or [string]$descriptor.ToolVersion -ne "5.0") { throw "Descriptor sai schema: $($descriptor.ModuleId)" }
