...
if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
    ...
    if ($manifestText -notmatch 'version="5\.0\.0\.0"') { $failures.Add('Application manifest sai phiên bản nội bộ Windows 5.0.0.0.') }
}
...
$versionChecks = @(
    ...
    @{ File='VietLicenSure-v5.0-OneFile.cs'; Pattern='AssemblyVersion\("5\.0\.0\.0"\)' },
    @{ File='VietLicenSure-v5.0-OneFile.cs'; Pattern='AssemblyFileVersion\("5\.0\.0\.0"\)' },
    @{ File='VietLicenSure-v5.0-OneFile.cs'; Pattern='AssemblyInformationalVersion\("5\.0"\)' }
)
...
if (-not (Test-Path -LiteralPath $applicationUpdateManifestPath -PathType Leaf)) {
    ...
    try {
        ...
        if ([string]$applicationUpdateManifest.ReleasePageUrl -ne 'https://github.com/thanhvietithopnghia-rgb/VietLicenSure/releases/tag/v5.0' -or
            [string]$applicationUpdateManifest.DownloadUrl -ne 'https://github.com/thanhvietithopnghia-rgb/VietLicenSure/releases/download/v5.0/VietLicenSure-v5.0.exe') {
